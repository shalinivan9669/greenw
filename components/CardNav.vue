...
const navigateToLink = (lnk?: CardNavLink) => {
  if (!lnk || !lnk.href) return

  const closeMenu = () => {
    const tl = tlRef.value
    if (tl && isExpanded.value) {
      isHamburgerOpen.value = false
      tl.eventCallback('onReverseComplete', () => {
        isExpanded.value = false
        tl.eventCallback('onReverseComplete', null)
  ...
  router.push(lnk.href).finally(closeMenu)
}
...
<template>
  <div
    :class="`card-nav-container pointer-events-auto absolute left-1/2 -translate-x-1/2 w-[90%] max-w-[900px] z-[99] top-[1.1em] md:top-[1.6em] ${props.className}`"
  >
    <nav
      ref="navRef"
      :class="[
        ...
        >
          ...
          <div class="flex flex-col gap-[2px] mt-auto nav-card-links">
            <component
              ...
              :key="`${item.label}-${lnk.label}-${i}`"
              :is="(lnk.href || '').startsWith('/') ? 'NuxtLink' : 'a'"
              ...
              :rel="lnk.href && lnk.href.startsWith('http') ? 'noopener noreferrer' : undefined"
              @click.prevent="navigateToLink(lnk)"
            >
              <svg
                class="nav-card-link-icon shrink-0 w-4 h-4"
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
                stroke-width="2"
