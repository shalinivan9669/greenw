      ...
      >
        <div :class="round ? 'p-0 m-0' : 'mb-4 p-5'">
          <span class="flex h-[28px] w-[28px] items-center justify-center rounded-full bg-[#0b0b0b]">
            <span v-if="item.iconText" class="text-white text-base leading-none">{{ item.iconText }}</span>
            <i v-else :class="item.icon" class="text-white text-base"></i>
          </span>
        </div>
...
const handleDragEnd = (_event: Event, info: DragInfo) => {
